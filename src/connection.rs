...
// ============================================================
// Legacy EslHandle (kept temporarily for migration)
// ============================================================

use std::collections::VecDeque;

/// ESL connection handle (legacy — use EslClient instead)
...
impl EslHandle {
    ...
    pub async fn connect(host: &str, port: u16, password: &str) -> EslResult<Self> {
        ...
        handle.legacy_authenticate(password).await?;
        info!("Successfully connected and authenticated to FreeSWITCH");
        Ok(handle)
    }
    ...
    pub async fn connect_with_user(
        ...
        handle.legacy_authenticate_user(user, password).await?;
        info!("Successfully connected and authenticated to FreeSWITCH");
        Ok(handle)
    }
    ...
    async fn legacy_authenticate(&mut self, password: &str) -> EslResult<()> {
        debug!("Starting authentication");

        let message = self.legacy_recv_message().await?;
        if message.message_type != MessageType::AuthRequest {
            return Err(EslError::protocol_error("Expected auth request"));
        }
    ...
    async fn legacy_authenticate_user(&mut self, user: &str, password: &str) -> EslResult<()> {
        debug!("Starting user authentication for user: {}", user);
    ...
    async fn legacy_recv_message(&mut self) -> EslResult<EslMessage> {
        loop {
